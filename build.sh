...
# 1. start container
#  ./build.sh --host-config-dir ~/work/zmk-config --host-zmk-dir ~/work/zmk --version stable
#
# 2. run build (IN CONTAINER)
# /bin/bash -c 'cd /workspace/zmk/app && west build -b nice_nano_v2 -- -DSHIELD="splitkb_aurora_lily58_left nice_view_adapter nice_view" -DZMK_CONFIG="/workspace/zmk-config/config"'
# (clean build)
# /bin/bash -c 'cd /workspace/zmk/app && west build --pristine -b nice_nano_v2 -- -DSHIELD="splitkb_aurora_lily58_left nice_view_adapter nice_view" -DZMK_CONFIG="/workspace/zmk-config/config"'
#
# 3. copy back
# scp $USER@172.16.15.2:/home/$USER/work/zmk/app/build/zephyr/zmk.uf2 ~/Downloads/lilly58_left.uf2
...
$DOCKER_CMD -w "$DOCKER_ZMK_DIR" -it "$DOCKER_IMG" $CMD
